...
DLURL="https://www.census.gov/ts/x13as/unix/x13ashtmlsrc_V1.1_B19.tar.gz"
...
# download
curl -s -k -L -O $DLURL
tar xzv -C /tmp/x13ashtmlsrc -f x13ashtmlsrc_V1.1_B19.tar.gz
...
# show dependend libaries: otool -L x13ashtml

# copy dependend libaries to designated folder
...
# libquadmath.0.dylib
install_name_tool -id @executable_path/../lib/libquadmath.0.dylib libgfortran.3.dylib
install_name_tool -change /usr/local/gfortran/lib/libgcc_s.1.dylib  @executable_path/../lib/libgcc_s.1.dylib libSystem.B.dylib
